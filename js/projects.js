/* Projects page scroll animations */

document.addEventListener('DOMContentLoaded', () => {
  const selectors = ['.projects-list', '.more-projs'];
  const nodes = Array.from(document.querySelectorAll(selectors.join(',')));
  if (!nodes.length) return;

  function getClassFor(el) {
    if (el.classList.contains('projects-list')) return 'inview-slide-right';
    if (el.classList.contains('more-projs')) return 'inview-zoom-in';
    return '';
  }
